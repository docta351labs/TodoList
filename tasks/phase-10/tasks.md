...
- [ ] **10.4** `useTodoLists.test.ts` (hook test with MSW mock server)
...
- [ ] **10.7** Create `src/tests/handlers/todoHandlers.ts` with MSW handlers for:
...
- [ ] **10.8** Create `src/tests/server.ts` — MSW `setupServer(...handlers)` instance

---

## E2E Test (Playwright — smoke only)

- [ ] **10.9** `smoke.spec.ts` (`tests/e2e/smoke.spec.ts`):
