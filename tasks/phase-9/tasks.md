...
- [ ] **9.1** Configure routes in `src/App.tsx` using React Router v7:
  ```tsx
  <Routes>
  ...
  ```
  *(No `/login` page needed in v1 — mock auth bypasses authentication)*
- [ ] **9.2** Create `src/pages/DashboardPage.tsx` — displays user's todo lists
- [ ] **9.3** Create `src/pages/ListDetailPage.tsx` — displays a single list with all its items

---

## Feature: `todos` — TanStack Query Hooks (`src/features/todos/api.ts`)

- [ ] **9.4** `useTodoLists()` — `useQuery` → `GET /todolists`
- [ ] **9.5** `useTodoList(listId: string)` — `useQuery` → `GET /todolists/:id`
- [ ] **9.6** `useCreateTodoList()` — `useMutation` → `POST /todolists`
  - Optimistic: append new list to cached list
  - Rollback on error
  - Invalidate `['todolists']` on settle
- [ ] **9.7** `useDeleteTodoList()` — `useMutation` → `DELETE /todolists/:id`
  - Optimistic: remove list from cached list immediately
  - Rollback on error
- [ ] **9.8** `useAddTodoItem(listId: string)` — `useMutation` → `POST /items`
  - Optimistic: append item with temp ID and `status: 'Pending'`
  - Rollback on error
  - Show toast on error with `traceId` from ProblemDetails
- [ ] **9.9** `useUpdateTodoItem(listId: string)` — `useMutation` → `PUT /items/:id`
- [ ] **9.10** `useUpdateTodoItemStatus(listId: string)` — `useMutation` → `PATCH /items/:id/status`
  - Optimistic: update item status in cache immediately
  - Rollback on error (invalid transition → show inline error)
- [ ] **9.11** `useDeleteTodoItem(listId: string)` — `useMutation` → `DELETE /items/:id`
...
- [ ] **9.15** `AddTodoItemForm` (`AddTodoItemForm.tsx` + `AddTodoItemForm.module.css`)
  - Inputs: title (required), description (optional), priority (select), due date (optional)
  - Inline validation for all fields
- [ ] **9.16** `TodoItemStatusControl` (`TodoItemStatusControl.tsx`)
...
- [ ] **9.17** `ErrorBoundary` — catches render errors; shows retry button; logs `traceId`
- [ ] **9.18** `Toast` — transient notification; auto-dismiss after 4s; shows error message + optional `traceId`
- [ ] **9.19** `LoadingSpinner` — centered spinner with accessible `aria-label`

---

## Feature Barrel Export

- [ ] **9.20** Create `src/features/todos/index.ts` — re-export all hooks and components
